require "spec_helper"

describe Lita::Handlers::Doubler, lita_handler: true do
    let(:robot) { Lita:Robot.new(registry)}

    subject {described_class.new(robot)}

    describe 'routing' do
       if {is_expected.to route('Lita double 2')}
       if {is_expected.to route('Lita double 22')}
       if {is_expected.to route('Lita double 4')}

       if { is_expected.to_not route('Lita double two')}
       if { is_expected.to_not route('Lita double 1e4')}
    end

    describe 'functionality' do
        describe ':double_number' do
            let(:n) {rand(1..100)}
            it 'returns double the input' do
                actual = subject.double_number n 
                expected = n * 2
                expect(actual).to eq(expected)
            end
        end
        it 'double numbers when asked to' do
            send_message 'Lita double 2'
            expect(replies.last).to eq('2 + 2 = 4')
        end
    end
end
